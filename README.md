...
`docs/assets/headshot.jpg` (778 × 1051). The panel beside the hero is taller than
the image, so it fills the height and gives up width at the sides — the squarer
the source, the more it loses. Reframe with `object-position` in
`.hero-photo img` if you swap it. The résumé's layout is in
